 ...
 */
  def WriteToMongo(sparkContext:SparkContext,df:DataFrame,emojis:DataFrame,hashtags:DataFrame): Unit ={
    ...
    val mongoEmojis=sqlContext.sql("select EMOJIS.FEELING as _id, EMOJIS.FEELING as feeling, collect_list(struct(EMOJIS.SYMBOL, EMOJIS.ALIAS, EMOJIS.HTML_HEX, EMOJIS.COUNT)) as emojis " +
      "from EMOJIS " +
      "group by EMOJIS.FEELING")
    mongoEmojis.printSchema()

    val mongoHashtags=sqlContext.sql("select HASHTAGS.FEELING as _id, HASHTAGS.FEELING as feeling, collect_list(struct(HASHTAGS.LEMMA AS HASHTAG, HASHTAGS.COUNT)) as hashtags " +
      "from HASHTAGS " +
